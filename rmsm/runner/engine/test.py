...
"""
    这里是验证集保存最终的测试结果的函数,包括单CPU测试和多个
"""
...
def single_gpu_test(model: nn.Module, data_loader: DataLoader) -> list:
    ...
    for data in data_loader:
        ...
        # refer to https://github.com/open-mmlab/rmsm/issues/985
...
def collect_results_cpu(result_part: list,
    ...
    rmsm.utils.dump(result_part, part_file)
    dist.barrier()
    # collect all parts
    if rank != 0:
        return None
        ...
        for i in range(world_size):
            ...
            part_result = rmsm.load(part_file)
