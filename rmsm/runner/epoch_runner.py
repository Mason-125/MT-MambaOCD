...
class EpochRunner(object):
    ...
    def train(self, data_loaders):
        ...
        for step, data_batch in enumerate(trainLoader):
            # data = data.to(device)  这个是调用GPU运行数据
            data = data_batch[0]
            label = data_batch[1]
            data = data.float()
            label = label.float()
    ...
    def run(self, data_loaders: List[DataLoader], workflow):
        ...
        for epoch in range(self._max_epochs):
            ...
            log_writer.add_scalar('Loss/train', float(train_loss), epoch)  # 可视化loss
            log_writer.add_scalar('Loss/eval', float(eval_loss), epoch)  # 可视化loss
