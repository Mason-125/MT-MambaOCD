...
import numpy as np
from rmsm.utils import print_log
...
class KFoldDataset:
    ...
    def __init__(self,
        ...
        raman_id = dataset.data_infos['raman_id']

        Group1 = [2, 3, 5, 6, 10]
        Group2 = [7, 8, 11, 13, 29]
        Group3 = [36, 42, 12, 20, 22]
        Group4 = [35, 41, 4, 23, 24]
        Group5 = [9, 15, 16, 28, 27]
        Group6 = [26, 18, 31, 17, 37]
        Group7 = [38, 25, 14, 39, 40]

        Group = [Group1, Group2, Group3, Group4, Group5, Group6, Group7]


        if (test_mode == 'val') | (test_mode == 'test'):
            indices_id = Group[fold]
            print("val")
            print(indices_id)
            results = [index for index, value in enumerate(raman_id) if value in indices_id]
            self.indices = results
        else:
            Group_train_val = Group[:fold] + Group[fold + 1:]
            indices_id = []
            for i in range(0, len(Group_train_val)):
                indices_id += Group_train_val[i]
            print("train")
            print(indices_id)
            indices_train = [index for index, value in enumerate(raman_id) if value in indices_id]
            self.indices = indices_train

        # dataset
        self.dataset.update_pipeline(pipeline, self.indices)
    ...
    def get_labels(self):
        dataset_labels = self.dataset.get_labels()
        labels = np.array([dataset_labels[idx] for idx in self.indices])
        ...
        if self.separate_eval:
            ...
            for dataset_idx, dataset in enumerate(self.datasets):
                ...
                print_log(
                    ...
                    f'{len(results_per_dataset)} data now',
                    logger=logger)
...
class ClassBalancedDataset(object):
    ...
    sampling strategy in `this paper`_, in each epoch, an data may appear
    multiple times based on its "repeat factor".

    .. _this paper: https://arxiv.org/pdf/1908.03195.pdf

    The repeat factor for an data is a function of the frequency the rarest
    category labeled in that data. The "frequency of category c" in [0, 1]
    is defined by the fraction of datas in the training set (without repeats)
    ...
    1. For each category c, compute the fraction :math:`f(c)` of datas that
       contain it.
    ...
    3. For each data I and its labels :math:`L(I)`, compute the data-level
       repeat factor
    ...
    def _get_repeat_factors(self, dataset, repeat_thr):
        # 1. For each category c, compute the fraction # of datas
        #   that contain it: f(c)
        ...
        for k, v in category_freq.items():
            assert v > 0, f'caterogy {k} does not contain any datas'
            category_freq[k] = v / num_datas
        ...
        # 3. For each data I and its labels L(I), compute the data-level
        # repeat factor:
        #    r(I) = max_{c in L(I)} r(c)
        repeat_factors = []
        for idx in range(num_datas):
