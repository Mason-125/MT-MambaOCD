...
# Define the iterative polynomial baseline correction method
def iterative_polynomial_baseline_correction(x, y, n_iter=10, degree=3):
    corrected_y = y.copy()
    for iteration in range(n_iter):
        peaks, _ = find_peaks(-corrected_y, height=0)
        if len(peaks) < 2:
            break
        peak_x = x[peaks]
        peak_y = corrected_y[peaks]
        coeffs = np.polyfit(peak_x, peak_y, degree)
...
def baseline(x_input, y_input, bir, method, **kwargs):
    ...
    method : str
        ...
        - "iter_poly": Baseline correction by improved iterative polynomial fitting with automatic threshold
        ...
        - "airPLS": (automatic) Baseline correction using adaptive iteratively reweighted penalized least squares. Analyst, 2010, 135(5): 1138-1146.
    ...
    if method == 'poly':
    ...
    elif method == 'iter_poly':
        niter = kwargs.get('niter', 10)
        poly_order = kwargs.get('polynomial_order', 3)
        x = yafit[:, 0]
        y = yafit[:, 1]

        for _ in range(niter):
            # An iterative polynomial fitting is performed
            coeffs = np.polyfit(x, y, poly_order)
            baseline_fitted = np.polyval(coeffs, x)

            # The estimated baseline is removed
            y -= baseline_fitted

        return y.reshape(-1, 1), Y_scaler.inverse_transform(baseline_fitted.reshape(-1, 1))
        ...
        # optional parameters
        lam = kwargs.get('lam', 1.0 * 10 ** 2)
        porder = kwargs.get('p', 1)
        ...
        for i in range(1, niter + 1):
            ...
            if (dssn < 0.001 * (abs(x)).sum() or i == niter):
