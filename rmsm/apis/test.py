...
def single_gpu_test(model,
    ...
    prog_bar = rmsm.ProgressBar(len(dataset))
    for i, data in enumerate(data_loader):
        ...
        if show or out_dir:
            ...
            spectrum = torch.squeeze(data['spectrum'])
            if show:
            ...
            batch_size = data['spectrum'].size(0)
...
def collect_results_cpu(result_part, size, tmpdir=None):
    ...
    if tmpdir is None:
        ...
        rmsm.mkdir_or_exist(tmpdir)
    # dump the part result to the dir
    rmsm.utils.dump(result_part, osp.join(tmpdir, f'part_{rank}.pkl'))
    dist.barrier()
    # collect all parts
    if rank != 0:
        return None
        ...
        for i in range(world_size):
            ...
            part_result = rmsm.utils.load(part_file)
